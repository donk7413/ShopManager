...
var PurseStrings = PurseStrings || (function () {
	...
	commandBuy = function (msg) {
        ...
        // !ps --buy --buyer|<buyer_id> --seller|<seller_id> --amt|50gp --item|Dagger~Weapons --weight|0,1 --prop|Blabla --mod|bloblo --desc|blibli --service|0 --treasure|0
		...
		if (buyer && isPursed(buyer.get('id')) && seller && isPursed(seller.get('id'))) {
            ...
            if (purchased) {
                ...
                if (sold) {
                    ...
                    if (giving) {
                        showDialog('Transaction Effectuée', seller.get('name'), 'Vous recevez ' + amountprice + ' de ' + buyer.get('name') + '.', seller.get('name'), true);
                        showDialog('Transaction Effectuée', buyer.get('name'), 'Vous donnez ' + amountprice + ' à ' + seller.get('name') + '.', buyer.get('name'), true);
                        adminDialog('Transaction Effectuée', buyer.get('name') + ' donne ' + seller.get('name') + ' ' + amountprice + '.');
                    } else {
                        if (isMerchant(buyer_token_id)) {
                            updateInventory(buyer_token_id, item, amount, 'add');
                            //adminDialog('Is Merchant Buyer','');
                ...
                showDialog('Erreur', buyer.get('name'), 'Vous n\'avez pas assez d\'argent pour cette transaction!', buyer.get('name'), true);
            }
      ...
      adminDialog('Info Record','item '+new_item +' weight ' + new_weight+' prop '+new_prop+' mod '+new_mod+' desc '+new_desc+' service '+isService);
        ...
        if (_.find(potions, function (x) { return x == new_item; }) && state['PURSESTRINGS'].useExtScripts) {
            ...
            if (char) {
                ...
                //donk7413
                var edit_mod = new_mod.replace(/#/g, ":");
                var objet = {
                    Name: new_item,
                    Weight: new_weight,
                    Properties: new_prop,
                ...
                if(isService == 0)
                {
                    ...
                    var oglItem = {
                        ...
                        alterItemCount: function alterItemCount(subject, item, rowId, modifierQuantity) {
                          ...
                          if(newCount == 0)
                          {
                              	...
                              	var attrName = '' + oglItem.ITEM_PREFIX + rowId + oglItem.NAME_SUFFIX;
                                deleteAttributes(subject, '' + oglItem.ITEM_PREFIX + rowId)
                    ...
                    if(isbuy == 1)
                    {
                     ...
                     oglItem.createOrIncrementItem(getObj('character', token.get('represents')), objet);
                    
                    }
                    else
                    {
                        
                        var subject = getObj('character', token.get('represents'));
                         var existingRowId = oglItem.getExistingRowId(subject, objet.Name);
                         if (existingRowId !== null) {
                            oglItem.alterItemCount(subject, objet, existingRowId, -1);
                         }
    ...
    deleteAttributes = function(charId, rowid) {
		
    
	var test =findObjs({type: 'attribute', characterid: charId.id}, {caseInsensitive: true});
	
    test.filter(attr => attr.get('name').toLowerCase().indexOf(rowid.toLowerCase()) === 0).forEach(attr => attr.remove());

	
    
	
    //test.forEach(attr => attr.remove());
	
		//t attr = findObjs({type: 'attribute', characterid: charId, name: rowid+'_itemname'}, {caseInsensitive: true});
		//attr.forEach(a =>  a.remove());
		
		/*
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+'_itemweight'}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.EQUIPPED_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.USEASARESOURCE_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.HASATTACK_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.PROPERTIES_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.MODIFIERS_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
		attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.CONTENT_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		
			attr = findObjs({type: 'attribute', characterid: charId, name: rowid+oglItsem.INVENTORYSUBFLAG_SUFFIX}, {caseInsensitive: true});
		attr.forEach(a =>  a.remove());
		*/
		...
		//attr.forEach(a =>  a.remove());
		adminDialog('Suppression d\'objet '+ rowid +" pour personnage "+ charId.name);
		//return attr;
	},
    ...
    commandInventory = function (msg) {
		...
		if (commands[2] && commands[2] !== '') {
            ...
            if (token) {
                 ...
                 if (token.get('aura2_radius').trim() == 'show-stock') showStock = true;
                if (token.get('aura2_radius').trim() == 'hide-stock') showStock = false;
                ...
                if (merchant && isPursed(merchant.get('id'))) {
                    ...
                    if (notes && notes[0].match(/^PurseStrings (Inventory|Menu)$/) !== null) {
                        ...
                        //adminDialog('test2',JSON.stringify(inv));
                        if (inv && inv.length > 0) {
        ...
        if (token) {
            ...
            if (notes && merchant && isPursed(merchant.get('id'))) {
                ...
                _.each(cats, function (cat) {
                    ...
                    let thisCat = _.filter(inv, function (item) { return item.category == cat; });
                    _.each(thisCat, function (item) {
                        newNotes += '<p>' + item.name + '|' + item.price + '|' + item.quantity + '|' + item.weight + '|' + item.prop + '|' + item.mod + '|' + item.desc + '|' + item.service + '|' + item.treasure +'</p>';
                    });
                });
    ...
    parseInventory = function (notesArray) {
        ...
        _.each(notesArray, function(line) {
            ...
            if (line.search(/\|/) > 0) {
                ...
                //log
                /*adminDialog('name',a[0]);
                adminDialog('price',a[1]);
                adminDialog('quantity',a[2]);
                adminDialog('weight',a[3]);
                adminDialog('prop',a[4]);
                adminDialog('mod',a[5]);
                adminDialog('desc',a[6]);
                adminDialog('isService',a[7]);*/
               ...
               if (a[5] != '') {
                ...
                if (a[6] != '') {
                 ...
                 if (a[7] != '' && !isNaN(a[7])) {
                    item.service = parseInt(a[7]);
                } else {
                    item.service = 0;
                }
                
                 if (a[8] != '' && !isNaN(a[8])) {
                    item.treasure = parseInt(a[8]);
                } else {
                    item.treasure = 0;
                }
